...
export default function Myposts() {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      ...
      try {
        const response = await axios.get("https://backend-final-3-g9lp.onrender.com/post/myPosts", {
          headers: {
  ...
  const handleDeletePost = async (postId) => {
    const token = localStorage.getItem("token");
    try {
      await axios.delete(`https://backend-final-3-g9lp.onrender.com/post/deletepost/${postId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
